...
export class UserReportComponent implements OnInit {
  constructor(readonly service: UserService , readonly authService: AuthService,
...
// });
  ...
  initialize() {
    const searchCriteria = new ClinetReportModel();
  ...
  bindColumns() {
    this.cols = [//Lead Number
      ...
      // { header: 'Store Name', field: 'Store Name' },
       { header: 'UserId', field: 'Id' },
      { header: 'Name', field: 'FirstName' },
      { header: 'Email Id', field: 'EmailId' },
      { header: 'Role', field: 'RoleId' },
      { header: 'Birth Date', field: 'BOD' },
  ...
  search(){
    ...
    if (this.searchForm.valid) {
     ...
     console.log(this.displayArr);
      ...
      this.service.getUserReport(this.searchData).subscribe((response: any) => {
  ...
  addWorker(){
    this.routeUrl.navigate(['/WorkerDetail'])
  }
  reset(){
